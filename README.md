...
- **No universal winner** (matching the operator-learning literature): FNO
  ...
  community rediscovers this regime structure end to end.
- **The discovered Darcy hybrid** (`attention–wavelet–residual_conv–fourier–wavelet`,
  1.3M params) attains the **best mean converged Darcy error in our entire
  baseline suite** (0.0631 ± 0.0083 vs. 0.0649 ± 0.0037 for DeepONet).
- **On Navier–Stokes the canonical answer stands**: the rule-based arm collapses
  to pure-Fourier stacks (the textbook solution), which remain unbeaten; the
  LLM's hybrid trails slightly at a matched parameter budget (0.0005 vs. 0.0002
  at ~1.1M params) and stays an order of magnitude below every non-FNO baseline.
- **Mechanism, not just outcomes**: across 4,817 logged planner decisions the
  LLM chose *hybridize* 99.8% of the time — the reason it produces multi-family
  hybrids where the analytic swarm produces canonical single-family designs.
...
- **Auditability**: 9,623 LLM calls across the campaign, logged with prompts,
  raw responses, and parse status — **zero silent rule-based fallbacks**.
- **Cost of agency**: LLM coordination costs 2–4× the wall-clock of rule-based
  coordination at identical training budget (quantified per PDE in the paper).
