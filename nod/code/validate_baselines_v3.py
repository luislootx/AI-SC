"""EXP-4 v3 — RESUME-SAFE honest baseline validation.
...
# v3-campaign winners (best LLM run per PDE; exact global_best_genome from
# the run's FINAL.json, including the discovered lr/wd -- the learning rate
# is part of the genome vocabulary, so faithfulness includes it).
V3_HYBRID_NS = ArchitectureGenome(          # v2_llm_ns_seed137
    block_sequence=["fourier", "attention", "wavelet", "branch_trunk",
                    "residual_conv", "fourier"],
    hidden_channels=32, fourier_modes=16, activation="gelu",
    use_gating=False, use_skip_connections=True, dropout_rate=0.0,
    num_blocks=6, learning_rate=1e-4, weight_decay=1e-5,
)
V3_HYBRID_DARCY = ArchitectureGenome(       # v2_llm_darcy_seed137
    block_sequence=["attention", "wavelet", "residual_conv", "fourier",
                    "wavelet"],
    hidden_channels=64, fourier_modes=10, activation="silu",
    use_gating=True, use_skip_connections=True, dropout_rate=0.031,
    num_blocks=5, learning_rate=2e-3, weight_decay=0.0,
)
...
def build_models(resolution: int) -> dict[str, ModelSpec]:
    return {
        ...
        "Discovered Hybrid":    ModelSpec("Discovered Hybrid",
            ...
            epochs=80, normalize_io=False),
        # v3-campaign winners, evaluated on BOTH regimes (cross-regime story)
        "Hybrid v3 (NS)":       ModelSpec("Hybrid v3 (NS)",
            lambda r: ConfigurableNeuralOperator(V3_HYBRID_NS),
            epochs=80, normalize_io=False, lr=1e-4, wd=1e-5),
        "Hybrid v3 (Darcy)":    ModelSpec("Hybrid v3 (Darcy)",
            lambda r: ConfigurableNeuralOperator(V3_HYBRID_DARCY),
            epochs=80, normalize_io=False, lr=2e-3, wd=0.0),
    }
...
def main():
    ...
    log("=" * 84)
    log(f"EXP-4 v3 (resume-safe)  device={DEVICE}  seeds={args.seeds}  pdes={args.pdes}")
    log("=" * 84)
