...
class SwarmConfig:
    ...
    # Re-tuned Apr 28 after validation showed efficiency gaming dominated.
    ...
    # PDE selector (EXP-3). "ns" = NavierStokes (default); also "darcy", "heat".
    pde_name: str = "ns"
...
def large_demo_config() -> SwarmConfig:
    """Scale-up config for EXP-2: 16 labs, 25 iter, 512 samples, unbounded fitness.
    Designed to test whether scaling breaks premature convergence."""
    return SwarmConfig(
        num_labs=16,
        num_iterations=25,
        resolution=32,
