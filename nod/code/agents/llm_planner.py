...
class LLMPlanner:
    ...
    def propose(self, *, lab, global_best_genome: Optional[ArchitectureGenome],
        ...
        try:
            self.backend._ctx = {"role": "planner", "iteration": iteration,
                                 "lab_id": lab.lab_id}
            data = self.backend.chat_json(
                PLANNER_SYSTEM, user, max_tokens=900, temperature=0.8)
            if data.get("_parse_error") or not isinstance(data.get("new_genome"), dict):
                # Empty/unparseable LLM output. Do an EXPLICIT analytic fallback
                # (and mark it as such) instead of silently resubmitting the
                # current genome -- otherwise the lab stalls while the run is
                # mislabeled as LLM-driven.
                self.last_action = "fallback"
                self.last_rationale = "llm_empty_or_unparseable"
                if self.log_calls:
                    print(f"    Lab {lab.lab_id} planner FALLBACK "
                          f"(empty/invalid LLM JSON); using PSO planner")
                if random.random() < lab.exploration_rate:
                    lab._explore()
                else:
                    lab._exploit(global_best_genome)
                return lab.current_genome
            new_genome = _coerce_genome(data["new_genome"], lab.current_genome)
