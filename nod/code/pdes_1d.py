...
class Burgers1D:
    ...
    def generate(self, num_samples: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        ins, outs = [], []
        for _ in range(num_samples):
            # Large-amplitude ICs can violate CFL and blow the explicit advection
            # term up to non-finite values (this corrupted the burgers seed=2024
            # dataset). Re-sample the IC until the solve is finite. This is a
            # no-op for well-behaved seeds (it breaks on the first attempt), so
            # clean datasets are byte-identical; only blow-up draws are replaced.
            u0 = self._random_ic()
