...
def run_resumable(community: AIScientificCommunity, *, run_dir: str,
    ...
    # Per-run LLM transcript: durable evidence that planning/review are
    # LLM-driven (every chat_json call is appended). Both the 1D and 2D
    # runners reuse run_resumable, so this covers all swarm runs.
    os.environ["LLM_TRANSCRIPT"] = os.path.join(run_dir, "llm_transcript.jsonl")
...
def main():
    ...
    p.add_argument("--agents", choices=["on", "off"], default=None,
                   help="override preset: on=LLM planner+reviewer, off=PSO/rules")
    args = p.parse_args()

    cfg = PRESETS[args.preset]()
    cfg.pde_name = args.pde
    if args.agents == "on":
        cfg.use_llm_planner = cfg.use_llm_reviewer = True
    elif args.agents == "off":
        cfg.use_llm_planner = cfg.use_llm_reviewer = False
